...
def save_results_table(y_real, y_pred_proba, results_path, window_days):
    ...
    result_table.to_csv(os.path.join(results_path, f"result_table_RF_{window_days}d.csv"))
    return result_table

def perform_bootstrap_analysis(nfeatures_t, labels_t, results_path, window_days, n_trials=5, alpha=0.05):
    """Perform bootstrap confidence interval analysis for RF model."""
    # Load saved RF model
    ...
    for key in all_CI.keys():
        ...
        df_ci.loc[len(df_ci)] = [
            lower, upper, f"({lower},{upper})", key, f"RF_0d_{window_days}d"
        ]
    
    # Save confidence intervals
    ci_filename = f"confidence_interval_RF_0d_{window_days}d.csv"
    ...
    for data, name in [(precision, 'precision'), (recall, 'recall')]:
        filename = f'{name}_RF_0d_{window_days}d.pkl'
...
if __name__ == "__main__":
    ...
    for idx_folder, specific_folder_name in enumerate(selected_list):
        ...
        if to_train_model:
            ...
            # Save model
            ckpt_dir = os.path.join(results_output_path, 'optuna_tuning', 'objective_RF_LT', f'0d_{window_days}d')
