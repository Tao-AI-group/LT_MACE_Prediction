...
def save_results_table(y_real, y_pred_proba, results_path, window_days):
    ...
    result_table.to_csv(os.path.join(results_path, f"result_table_LGBM_{window_days}d.csv"))
    return result_table

def perform_bootstrap_analysis(nfeatures_t, labels_t, results_path, window_days, n_trials=500, alpha=0.05):
    """Perform bootstrap confidence interval analysis for LGBM model."""
    # Load saved LGBM model
    ...
    for key in all_CI.keys():
        ...
        df_ci.loc[len(df_ci)] = [
            lower, upper, f"({lower},{upper})", key, f"LGBM_0d_{window_days}d"
        ]
    
    # Save confidence intervals
    ci_filename = f"confidence_interval_LGBM_0d_{window_days}d.csv"
    ...
    for data, name in [(precision, 'precision'), (recall, 'recall')]:
        filename = f'{name}_LGBM_0d_{window_days}d.pkl'
...
if __name__ == "__main__":
    ...
    for idx_folder, specific_folder_name in enumerate(selected_list):
        ...
        if to_train_model:
            ...
            params = {
                "n_estimators" :  1000,
                "learning_rate" : 0.28667337189426784,
                "max_depth" : 10,
                "reg_alpha" : 40,
                "reg_lambda" : 90,
                "num_leaves" : 900,
            ...
            # Save model
            ckpt_dir = os.path.join(results_output_path, 'optuna_tuning', 'objective_LGBM_LT', f'0d_{window_days}d')
