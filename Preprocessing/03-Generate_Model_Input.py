...
WINDOW_LABEL_MAP = {
    30: 15,    # 0-30 days label
    365: 16,   # 0-365 days label
    1095: 17,  # 0-1095 days label
    1825: 18,  # 0-1825 days label
}

# File type configurations
FILE_CONFIGS = {
    'age': {'cols': (1, 0, 20), 'prefix': 'A_', 'source': 'encounters'},
    'gender': {'cols': (1, 0, 13), 'prefix': 'G_', 'source': 'encounters'},
    'diagnosis': {'cols': (0, 1, 2), 'prefix': '', 'source': 'diagnosis'},
    'medication': {'cols': (0, 1, 2), 'prefix': '', 'source': 'medications'},
    'procedure': {'cols': (0, 1, 2), 'prefix': '', 'source': 'procedures'}
}
...
def get_qualified_patients(input_path: str, prediction_prediction_start_daygap: int) -> tuple[set[str], dict]:
   """
   ...
   try:
       with open(input_path, 'r') as input_file:
           ...
           for row in csv_reader:
               ...
               day_gap = row[19]    # Day gap for encounter
       ...
       for patient_id, gap_list in patient_daygaps.items():
           ...
           if last_gap >= prediction_prediction_start_daygap:
...
def get_qualified_input_encid_dic(outcome_input_path: str, qualified_patients: set[str], prediction_start_daygap: int, days_before_index: int) -> dict:
    """
    ...
    try:
        with open(outcome_input_path, 'r') as f:
            ...
            for row in csv_reader:
                ...
                if -days_before_index <= day_gap <  prediction_start_daygap:
                    encounter_id = row[0]
                    encounter_date = row[22].split(' ')[0]
...
def get_case_control_set(
    ...
    """
    case_patients = set()  # Fixed: changed case_pt_set to case_patients
    control_patients = set()  # Fixed: changed control_pt_set to control_patients
...
def get_split_file_paths(base_dir: str) -> Tuple[str, str, str, str, str, str]:
    ...
    return (
        ...
        'NA',  # type_file
        os.path.join(base_dir, 'lt'),  # out_file
        'binary',  # cls_type
        'NA'  # pts_file_pre
    )
...
if __name__ == "__main__":
    ...
    input_files = {
        ...
        'medications': outcome_input_common_path + 'mod_drug.csv',
        'procedures': outcome_input_common_path + 'mod_proc.csv'
    }

    # 2. Setup output paths
    output_folder_name = 'Results_15_final_new'
    output_base = get_common_path('../../Results/') + output_folder_name + '/'
    ...
    for window_name, window_params in prediction_windows.items():
        prediction_start_daygap, prediction_end_daygap = window_params
        start_time = dt.now()
        count_file += 1
        ...
        print(f"Window: {prediction_start_daygap}d to {prediction_end_daygap}d\n")
        
        # Create output directory for this window
        window_dir = f"{output_base}/{days_before_index}/Results_{prediction_start_daygap}d_{prediction_end_daygap}d_window/"
        ...
        for idx1, (patients, output_filename) in enumerate(patient_groups):
            print(f"{count_file}-{idx1+1}. Start generating {output_filename} for Results_{prediction_start_daygap}d_{prediction_end_daygap}d_window")
        ...
        print(f"Generating train/test/valid for Results_{prediction_start_daygap}d_{prediction_end_daygap}d_window")
